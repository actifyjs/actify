...
const Slider = forwardRef((props, ref) => {
  ...
  return (
    <div {...rest} className={cn('relative h-[68px] pt-[48px] pb-[18px] pointer-events-none', className)}>
      ...
      />
      {/* label */}
      <i style={{ left: `calc(${percent}% - 14px)` }} className="select-none absolute left-0 top-0 opacity-80">
        ...
        <span className="absolute inset-0 pt-2 text-center text-xs not-italic text-white">{value}</span>
      </i>
      <div className="h-1 rounded-full overflow-hidden bg-black/20 dark:bg-white/20"></div>
      {/* active track */}
      <i
        style={{ width: `calc(${percent}% - 10px)` }}
        className="rounded-l-full absolute left-0 top-[48px] h-1 bg-current opacity-80"
      />
      {/* handle circle */}
      <i
      ...
      <i
        ...
        className="select-none absolute top-[28px] block h-10 w-10 rounded-full peer-focus:bg-black/25 dark:peer-focus:bg-white/25"
      />
    </div>
  )
})
